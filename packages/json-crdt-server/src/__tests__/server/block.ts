  ...
  describe('block.*', () => {
    describe('block.new', () => {
      test('can create an empty block', async () => {
        ...
        expect(model2.clock.sid).toBe(SESSION_GLOBAL);
        await stop();
      });
      ...
      test('should throw CONFLICT error if block already exists', async () => {
        const {call, stop} = await setup();
        const id = getId();
        ...
        try {
          ...
          expect(err!.code || err!.message).toBe('CONFLICT');
        }
        await stop();
      });
    });
    ...
    if (!params.staticOnly) {
      describe('block.listen', () => {
        ...
        test('can filter out own edits', async () => {
          const {call, call$, stop} = await setup();
          const id = getId();
          ...
          await tick(11);
          const emits: TBlockEvent[] = [];
          call$('block.listen', {id, clientId}).subscribe(({event}) => emits.push(event));
          const model = Model.create();
          model.api.root({
            text: 'Hell',
          });
    ...
    describe('block.pull', () => {
      ...
      test('can create a new block, if it does not exist', async () => {
        ...
        expect(model.clock.sid).toBe(SESSION_GLOBAL);
      ...
      test('when client SEQ is ahead in the future, return latest snapshot for client to hard reset', async () => {
        const {call, stop} = await setup();
        const id = getId();
        const model = Model.create();
        model.api.root({
        ...
        const pull = await call('block.pull', {id, seq: 100});
        const model2 = Model.load(pull.snapshot!.blob);
        for (const batch of pull.batches)
          for (const patch of batch.patches) model2.applyPatch(Patch.fromBinary(patch.blob));
        expect(model.view()).toEqual({text: 'Hello World'});
        expect(model2.view()).toEqual({text: 'Hello World'});
        await stop();
      });

      test('when block does not exist, returns NOT_FOUND error', async () => {
        const {call, stop} = await setup();
        const id = getId();
        const [, err] = await of(call('block.pull', {id, seq: 100}));
        expect(err).toMatchObject({code: 'NOT_FOUND'});
        await stop();
      });

      test('when block was deleted, returns NOT_FOUND error', async () => {
        const {call, stop} = await setup();
        const id = getId();
        const model = Model.create();
        model.api.root({
    ...
    describe('block.del', () => {
      ...
      test('can delete a block', async () => {
        const {call, stop} = await setup();
        const id = getId();
        const [, err] = await of(call('block.get', {id}));
        expect(err).toMatchObject({code: 'NOT_FOUND'});
        await call('block.new', {id});
        await call('block.get', {id});
        await call('block.del', {id});
        const [, err2] = await of(call('block.get', {id}));
        expect(err2).toMatchObject({code: 'NOT_FOUND'});
        await call('block.del', {id});
        const [, err3] = await of(call('block.get', {id}));
        expect(err3).toMatchObject({code: 'NOT_FOUND'});
        await stop();
      });

      test('can delete a block and create a new one with the same ID', async () => {
        const {call, stop} = await setup();
        const id = getId();
        const [, err] = await of(call('block.get', {id}));
        expect(err).toMatchObject({code: 'NOT_FOUND'});
        const create = async (val: unknown) => {
          ...
          model.api.root(val);
          const patch = model.api.flush();
          await call('block.new', {id, batch: {patches: [{blob: patch.toBinary()}]}});
        };
        const get = async () => {
          const res = await call('block.get', {id});
        ...
        const model1 = await get();
        expect(model1.view()).toEqual({text: 'Hello'});
        await call('block.del', {id});
        const [, err2] = await of(get());
        expect(err2).toMatchObject({code: 'NOT_FOUND'});
        await create({x: 'y'});
        const model2 = await get();
        expect(model2.view()).toEqual({x: 'y'});
        await stop();
      });

      test('can delete a block with history and create a new one with the same ID', async () => {
        const {call, stop} = await setup();
        const id = getId();
        const [, err] = await of(call('block.get', {id}));
        expect(err).toMatchObject({code: 'NOT_FOUND'});
        ...
        const model0 = await create({text: 'Hello'});
        model0.api.str(['text']).ins(5, ' World');
        await update(model0.api.flush());
        const model1 = await get();
        expect(model1.view()).toEqual({text: 'Hello World'});
        await call('block.del', {id});
        const [, err2] = await of(get());
        expect(err2).toMatchObject({code: 'NOT_FOUND'});
        await create({x: 'y'});
