...
describe('slices', () => {
  describe('inserts', () => {
    ...
    test('intersecting slice chunks point to two slices', () => {
      ...
      expect(point1.layers[0].data()).toEqual({emphasis: true});
      const point2 = next(point1)!;
      expect(point2.layers.length).toBe(3);
      expect(point2.layers[0].data()).toEqual(undefined);
      expect(point2.layers[1].data()).toEqual({emphasis: true});
      expect(point2.layers[2].data()).toEqual({bold: true});
      const point3 = next(point2)!;
      expect(point3.layers.length).toBe(2);
      expect(point3.layers[0].data()).toEqual(undefined);
      expect(point3.layers[1].data()).toEqual({bold: true});
