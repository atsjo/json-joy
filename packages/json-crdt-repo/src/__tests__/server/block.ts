...
import type {TBlockEvent} from '@jsonjoy.com/json-crdt-server/lib/routes/block/schema';
import type {JsonCrdtTestSetup} from '../setup';
  ...
  describe('block.*', () => {
    ...
    describe('block.pull', () => {
      ...
      test('can create a new block, if it does not exist', async () => {
        ...
        expect(get1[1]).toMatchObject({message: 'Not Found'});
