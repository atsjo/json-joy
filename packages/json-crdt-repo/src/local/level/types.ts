...
export interface BlockMeta {
   ...
   * store only the latest state model. If history tracking is enabled the block
  ...
  /**
   * Timestamp when the last time the block with successfully or unsuccessfully
   * synced with the remote.
   */
  syncTs?: number;

  /**
   * The number of consecutive sync failures, if any.
   */
  syncFailures?: number;
}
